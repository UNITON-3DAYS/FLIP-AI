...
PAGE_PROMPT = (
    ...
    "- 객관식: 각 표시마다 '앵커'(펜이 의도적으로 착지한 점)를 찾아 그 앵커에 가장 가까운 선택지 번호 "
    "하나를 고른다 — 체크(✓/∨)는 꺾인 점, 동그라미는 원 안쪽 중심, 슬래시는 선의 가운데가 앵커다. "
    "앵커에서 뻗어 나온 획(체크의 팔·동그라미 진입선·꼬리)이 옆이나 위아래 다른 번호를 스치거나 그 위로 "
    "지나가도 그 번호는 고른 게 아니다 — 앵커가 놓인 번호만 고른다. 번호 오른쪽 답 값·글자(예: \"27\") 위를 "
    "지난 획, 인쇄된 원문자도 제외. 보통 한 문제엔 한 개다. 선택지에 아무 표시가 없고 빈 공간에 고른 번호를 "
    "손으로 적었으면 그 숫자를 답으로 본다. 서로 독립된 앵커가 둘 이상 뚜렷할 때만 쉼표로 복수.\n"
    "- 주관식: 손글씨 값을 선형표기로(분수 a/b, 제곱근 sqrt(x), 거듭제곱 x^2).\n"
    "- 보기(ㄱㄴㄷㄹ)나 이름에서 '모두 고르시오'형: 학생이 빈 공간(보기 박스 아래·옆)에 손으로 쓴 답을 읽어라. "
    "손글씨 ㄱ은 인쇄체와 달리 모서리 획(⌐) 모양이고 보기 박스에서 떨어져 있을 수 있으니 stray선으로 무시하지 "
    "말 것. 보기 기호는 그대로(ㄱ,ㄴ), 이름은 이름대로. 여러 개면 쉼표.\n"
    "- 표시가 전혀 없는 문제는 빈 문자열.\n"
